...
volatile uint8_t counting_base_timer_period;
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
	if (htim->Instance == TIM2) {
		if (STATE == OPEN) {
			counting_base_timer_period++;
			if (counting_base_timer_period % 2 == 0) {
			...
			if (STATE == SECURING && counting_base_timer_period == 10) {
			...
			counting_base_timer_period++;
			BSP_LED_Toggle(LED_GREEN);
			if (STATE == OPENING && counting_base_timer_period % 12 == 0) {
