...
int main() {
	...
	HAL_UART_Receive_IT(&uart, &single_char, 1);
	init_gpio_led();
	int bool_base_case = 1;
	while (1) {
		if (INPUT == on) {
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *husart) {
	if (husart->Instance == uart.Instance) {
		if (single_char != '\r' && index_counter < sizeof(one_line) - 1) {
